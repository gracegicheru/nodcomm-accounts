...
class LoginController extends Controller
{
    ...
    public function showLoginForm()
    {
		if (isset($_GET['next'])) {
			...
			$next = 'https://console.nodcomm.com/dashboard';
			}else{
			 $next = 'https://dashboard.nodcomm.com/dashboard';	
			}*/
			$next = 'none';
		}
		session(['next' => $next]);
//        return view('auth.login')->with('next',$next);
        return view('auth.login12')->with('next',$next);
	...
	if ($this->hasTooManyLoginAttempts($request)) {
	 ...
	 if($request->email==''){
		...
		if($existingsuperadmin->first()->phone=='demo'){
			...
			return response(['status'=>'success','details'=>$next]);
		}else{
		...
		if($this->notification->send_sms($smsno,$sms_message)==true){
		try{
 ...
 public function verification(Request $request)
	{
		if (Session::has('next') && Session::get('next')!='none') {
			$next = Session::get('next');
		}else{
			$existinguser=User::where('email', '=', Session::get('loginemail'))->first();
			$existingsuperadmin=User::where('name', '=', $request->email)->where('admin', 1)->where('company_id', 0)->first();
			if((!empty($existinguser) && $existinguser->company_id==0) || !empty($existingsuperadmin)){
			$next = 'https://console.nodcomm.com/dashboard';
			}else{
			 $next = 'https://dashboard.nodcomm.com/dashboard';	
			}
		}
//        return view('auth.verify')->with('next',$next);
        return view('auth.verification')->with('next',$next);
	}
		...
		if($part1!='' && $part2!='' && $part3!='' && $part4!='' && $part5!='' && $part6!=''){
			...
			if($User->get()->isEmpty()){
			 ...
			 if($User->count()>0){
				...
				return response(['status'=>'success','details'=>$request->next]);
