...
//			return view('registration_step4')->with($data);
                return view('register4')->with($data);
        ...
        try{
			...
			$company->code_expiry_time = time();
            $company->code =$code;
			$company->extende_trial_date = $date;
			$company->save();
        ...
        try{
			...
			Session::forget('phone');
			Session::forget('step2');
			return response(['status'=>'success','details'=>url('/register/step4'),'phone'=>$request->input('phone'),'code'=>$code, 'finalCode'=>$final_code,'password'=>$password,'step3'=>'no']);
			}else{
			return response(['status'=>'error', 'details'=>'An error occurred, please try again.']);
			}
			}
		...
		if($part1!='' && $part2!='' && $part3!='' && $part4!='' && $part5!='' && $part6!=''){
			...
			if($Company->get()->isEmpty()){
			 ...
			 if($Company->count()>0){
				...
				return response(['status'=>'success','details'=>'https://console.nodcomm.com/dashboard','phone'=>$Company->first()->phone]);
